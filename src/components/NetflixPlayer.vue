  ...
  >
    ...
    <div class="nfp-screen">
      <div class="nfp-slide-panel">
        <component
          :is="slides[current].component"
          :key="current"
          v-bind="slides[current].props || {}"
        />
      </div>
    </div>
    ...
    <Transition name="nfp-fade">
      ...
      >
        ...
        <div class="nfp-title-area">
          ...
          <div class="nfp-ep-title">
            {{ slides[current].navLabel }} &nbsp;·&nbsp; 第 {{ current + 1 }} /
            {{ slides.length }} 頁
          </div>
        </div>
      </div>
    ...
    <div class="nfp-controls-zone" @mouseenter="onEnter" @mouseleave="onLeave">
      <Transition name="nfp-fade">
        <div v-if="controlsVisible" class="nfp-controls">
          <!-- Progress bar -->
          <div class="nfp-progress-wrap">
            <div class="nfp-progress-bar" @click="seekByClick">
              <div class="nfp-progress-bg"></div>
              <div
                class="nfp-progress-fill"
                :style="{ width: progressPct + '%' }"
              ></div>
              <div
                class="nfp-progress-thumb"
...
.nfp-progress-bar:hover .nfp-progress-fill {
  height: 6px;
}
.nfp-progress-bar:hover .nfp-progress-thumb {
  opacity: 1;
}
.nfp-progress-bg {
  position: absolute;
...
/* ---- Transitions ---- */
.nfp-fade-enter-active,
.nfp-fade-leave-active {
...
@media (max-width: 480px) {
  ...
  .nfp-btn-right {
    display: none;
  } /* hide decorative right controls on tiny screens */
}
</style>
